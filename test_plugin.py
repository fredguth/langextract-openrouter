#!/usr/bin/env python3
"""Test script for openrouter provider (Step 5 checklist)."""
...
try:
    from langextract_openrouter import openrouterLanguageModel
except ImportError:
...
PROVIDER_CLS_NAME = "openrouterLanguageModel"
PATTERNS = ['^openrouter']
...
def _example_id(pattern: str) -> str:
    ...
    return f"{base}/google/gemini-2.5-flash"

sample_ids = [_example_id(p) for p in PATTERNS]
sample_ids.append("unknown-model")

print("Testing openrouter Provider - Step 5 Checklist:")
print("-" * 50)
...
try:
    ...
    provider = openrouterLanguageModel(model_id=model_id)
